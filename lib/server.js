...
server.spawnChrome = function() {
	return new Promise((resolve, reject) => {
		...
		this.chromeChild = spawn(location, ['--headless', '--disable-gpu', '--remote-debugging-port=9222', '--no-sandbox']);
...
server.onRequest = function(req, res) {
	...
	this.firePluginEvent('requestReceived', req, res)
		.then(() => {
			...
			return req.prerender.chrome.Runtime.evaluate({expression: 'window.localStorage.clear()'});
		}).then(() => {
			this.finish(req, res);
		}).catch(() => {
			this.finish(req, res);
				...
				if (params.redirectResponse) {
					...
					if (params.redirectResponse.url === req.prerender.url && !this.shouldFollowRedirects(req)) {
						...
						Page.stopLoading().catch((err) => {
							console.log('err calling stopLoading', err);
							reject();
						});
					}
				}
...
server.loadUrlThenWaitForPageLoadEvent = function(req, res) {
	...
	return new Promise((resolve, reject) => {
		...
		Promise.all([
			...
			if(req.prerender.shouldClearCookies === false) {
				return Promise.resolve();
			}
			...
			setTimeout(() => {
				if (!finished) {
					finished = true;
					util.log('page timed out', req.prerender.url);
					resolve();
				}
			}, pageLoadTimeout);
...
server.parseHtmlFromPage = function(req, res) {
	return new Promise((resolve, reject) => {
		...
		req.prerender.chrome.Runtime.evaluate({
			...
			req.prerender.documentHTML = resp.result.value;
			return req.prerender.chrome.Runtime.evaluate({
			...
			req.prerender.documentHTML = doctype + req.prerender.documentHTML;
			resolve();
		}).catch((err) => {
...
server.finish = function(req, res) {
	if (req.prerender.chrome && req.prerender.chrome.target) {
		CDP.Close({
			id: req.prerender.chrome.target.id
		}).catch(() => {
